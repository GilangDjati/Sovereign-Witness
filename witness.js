 ...
 */
async function Mata(imagePath) {
    console.log(`\n[Mata] 👀 Initializing vision system...`);

    if (!fs.existsSync(imagePath)) {
        throw new Error(`[Mata] ❌ Cannot find image at ${imagePath}. Please provide a valid receipt.jpg for testing!`);
    }

    try {
        console.log(`[Mata] 🧠 Loading @qvac/sdk OCR model...`);
        ...
        console.log(`[Mata] ⚙️ Processing image through neural network...`);
        ...
        console.log(`[Mata] ✅ Extraction complete!\n`);
        console.log(`=========================================`);
        ...
        console.error(`[Mata] ❌ OCR failed:`, error);
        throw error;
    }
}
 ...
 */
async function Otak(ocrText) {
    console.log(`\n[Otak] 🧠 Activating analytical engine...`);

    let llmModelId;
    try {
        console.log(`[Otak] 🤖 Initializing @qvac/sdk lightweight LLM (1B)...`);
        console.log(`[Otak] 🔧 Configuring Model Constraints -> Execution: Pure CPU | Context: 1024 tokens`);
        ...
        llmModelId = await loadModel({
            ...
            onProgress: (p) => {
                ...
                if (p.loaded === p.total && p.total > 0 && !hasLoggedBoot) {
                    hasLoggedBoot = true;
                    console.log(`\n[Otak] ⏳ Model pulled. Booting into RAM (initFromConfig)... this may take a moment.`);
                }
            }
        });
        
        console.log(`[Otak] ✅ Model successfully booted into memory!`);
...
Text: "${ocrText}"`;
        ...
        console.log(`[Otak] ⚙️ Prompting LLM for structured extraction...`);
        ...
        console.log(`[Otak] 📝 Raw LLM output: ${rawOutput}`);
        ...
        try {
            ...
            if (parsedData.error || !parsedData.amount_usdt) {
            ...
            if (isNaN(amount) || amount <= 0) {
                console.error(`\n[Otak Safety Guard] 🚫 Triggered! Invalid or negative amount parsed: ${parsedData.amount_usdt}`);
            ...
            if (parsedData.recipient_address) {
                ...
                if (invalidNetworkName) {
                    console.error(`\n[Otak Safety Guard] 🚫 Triggered! ${invalidNetworkName} attempt blocked: ${parsedData.recipient_address}`);
                    throw new Error(`Invalid Network Exception: ${invalidNetworkName}`);
                }
                
                if (parsedData.recipient_address.length < 40) {
                    console.error(`\n[Otak Safety Guard] 🚫 Triggered! Cross-Chain attempt blocked: ${parsedData.recipient_address}`);
                    throw new Error("Invalid Network Exception: Short/Unknown");
                }
                
                const solanaAddressRegex = /^[1-9A-HJ-NP-Za-km-z]{32,44}$/;
                if (!solanaAddressRegex.test(parsedData.recipient_address)) {
                    console.log(`\n[Otak] ⚠️ Address failed Base58 check. Attempting heuristic correction...`);
                    ...
                    if (!solanaAddressRegex.test(parsedData.recipient_address)) {
                        console.error(`\n[Otak Safety Guard] 🚫 Triggered! Invalid Solana address format detected: ${parsedData.recipient_address}`);
                        throw new Error("Safety Guard Exception");
                    } else {
                        console.log(`[Otak] ✅ Heuristic correction successful! Cleaned Address: ${parsedData.recipient_address}`);
                    }
                }
            }
            ...
            for (const word of forbiddenWords) {
                ...
                if (regex.test(ocrText)) {
                    console.error(`\n[Otak Safety Guard] 🚫 Triggered! Dangerous OCR typo '${word}' detected as an isolated word.`);
                    throw new Error("Safety Guard Exception");
                }
            }
            ...
            console.error(`\n[Otak] ❌ Data Extraction Failed: The AI or Safety Guard detected messy or unreadable OCR text.`);
            console.log(`\n=========================================`);
        ...
        console.log(`[Otak] ✅ Analysis complete! Extracted Data:`, parsedData);
 ...
 */
async function Tangan(transactionData) {
    console.log(`\n[Tangan] 🖐️ Preparing to execute physical transaction on Solana...`);

    try {
        const { amount_usdt, purpose, recipient_address } = transactionData;

        console.log(`[Tangan] 🔗 Initializing local WDK instance and registering Solana Wallet...`);
        ...
        console.log(`[Tangan] 📝 Derived Sender Address: ${address}`);
        console.log(`[Tangan] 📝 Drafting transaction...`);
        console.log(`[Tangan]    -> Blockchain: Solana (Devnet)`);
        console.log(`[Tangan]    -> Asset: USDT`);
        console.log(`[Tangan]    -> Amount: ${amount_usdt}`);
        console.log(`[Tangan]    -> Recipient: ${recipient_address || "Default/Demo Address"}`);
        console.log(`[Tangan]    -> Memo/Purpose: ${purpose}`);

        console.log(`[Tangan] ✅ Transaction draft prepared successfully! Pending signature.`);
        ...
        console.error(`[Tangan] ❌ Transaction drafting failed:`, error);
        throw error;
    }
}
...
async function runSovereignWitness() {
    ...
    try {
        ...
        const ocrText = await Mata(receiptPath);
        const extractedData = await Otak(ocrText);
        await Tangan(extractedData);
